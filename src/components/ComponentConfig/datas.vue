  ...
  <el-form size="small" label-position="left">
    ...
    <template v-if="statusList[index].dataType == 'dynamicsData'">
      <el-form-item label="接口地址">
        <el-input
          v-model="statusList[index].request.url"
          :rows="2"
          @focus="handleInput"
        />
      </el-form-item>
...
const dataTypes: array = [
  { label: "静态数据", value: "staticData" },
  { label: "动态数据", value: "dynamicsData" },
];
//定义弹窗显隐
...
const handleShowParmsPop = () => {
  //判断是否仓储中有参数对象
  if (
    statusList.value[index.value].request &&
    Object.keys(statusList.value[index.value].request.params).length !== 0
  ) {
    //处理参数对象变成table数据格式
    ...
    for (let i in params) {
      arr.push({
        key: i,
        value: params[i],
        switch: params[i].components ? true : false,
      });
    }
    tableData.value = arr;
  } else {
