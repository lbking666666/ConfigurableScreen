<template>
  <div class="main-box-wrapper" ref="mainBox">
    <div
      ...
      :key="index"
      :class="{ 'main-col' :item.name !== 'Layouts', 'man-layout':item.name == 'Layouts','status':props.status=='edit', 'hover':comIndex == item.comKey}"
...
const handleDrag = (evnet: any,index:Number) => {
  ...
  //当前的left
  let curX:number = arr[index].style.left ? parseInt(arr[index].style.left) : 0;
  //当前的top
